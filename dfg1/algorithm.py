...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if param == "BPIC15_3f":
            dfg, start_activities, end_activities = pm4py.discover_dfg(BPIC15_3f, case_id_key='case:concept:name', activity_key='concept:name', timestamp_key='time:timestamp')
            ...
            dfg, start_activities, end_activities = pm4py.discover_dfg(BPIC17_f, case_id_key='case:concept:name', activity_key='concept:name', timestamp_key='time:timestamp')

        elif param == "RTFMP":
            dfg, start_activities, end_activities = pm4py.discover_dfg(RTFMP, case_id_key='case:concept:name', activity_key='concept:name', timestamp_key='time:timestamp')

        elif param == "SEPSIS":
            dfg, start_activities, end_activities = pm4py.discover_dfg(SEPSIS, case_id_key='case:concept:name', activity_key='concept:name', timestamp_key='time:timestamp')
