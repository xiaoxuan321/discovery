...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if param == "BPIC15_3f":
            log_skeleton = pm4py.discover_log_skeleton(BPIC15_3f, noise_threshold=0.1, activity_key='concept:name', case_id_key='case:concept:name', timestamp_key='time:timestamp')
            ...
            log_skeleton = pm4py.discover_log_skeleton(BPIC17_f, noise_threshold=0.1, activity_key='concept:name', case_id_key='case:concept:name', timestamp_key='time:timestamp')

        elif param == "RTFMP":
            log_skeleton = pm4py.discover_log_skeleton(RTFMP, noise_threshold=0.1, activity_key='concept:name', case_id_key='case:concept:name', timestamp_key='time:timestamp')

        elif param == "SEPSIS":
            log_skeleton = pm4py.discover_log_skeleton(SEPSIS, noise_threshold=0.1, activity_key='concept:name', case_id_key='case:concept:name', timestamp_key='time:timestamp')
